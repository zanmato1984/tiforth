...
- local shared-kernel executable coverage now includes the first collation-sensitive string checkpoint plus the first `struct<a:int32, b:int32?>`, `map<int32, int32?>`, and `dense_union<i:int32, n:int32?>` passthrough checkpoints, while the JSON slice remains a docs-first follow-on boundary with stable conformance, differential, adapter, and artifact-carrier anchors
