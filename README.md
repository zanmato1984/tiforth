...
- local shared-kernel executable coverage now includes the first `struct<a:int32, b:int32?>`, `map<int32, int32?>`, and `dense_union<i:int32, n:int32?>` passthrough checkpoints, while JSON and collation-sensitive string slices remain docs-first follow-on boundaries with stable conformance, differential, adapter, and artifact-carrier anchors
