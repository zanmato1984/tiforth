...
- executable differential coverage exists for `first-expression-slice`, `first-filter-is-not-null-slice`, `first-temporal-date32-slice`, `first-temporal-timestamp-tz-slice`, `first-decimal128-slice`, `first-float64-ordering-slice`, `first-unsigned-arithmetic-slice`, `first-json-slice`, and `first-collation-string-slice`; baseline-versus-exchange parity coverage exists for `first-exchange-slice`
...
- docs-first checkpoint anchors and planned differential artifact carriers now exist for `first-struct-slice`, `first-map-slice`, `first-union-slice`, and `first-exchange-slice`
