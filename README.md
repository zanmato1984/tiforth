...
- local shared-kernel executable coverage now includes the first signed-widening `add<int64>` checkpoint, the first JSON and collation-sensitive string checkpoints, plus the first `struct<a:int32, b:int32?>`, `map<int32, int32?>`, and `dense_union<i:int32, n:int32?>` passthrough checkpoints
