...
usage() {
  cat <<'USAGE'
Usage:
  scripts/refresh-first-temporal-timestamp-tz-tikv-live-artifacts.sh [--write-artifacts] [--dry-run]

Modes:
  default            run live first-temporal-timestamp-tz TiKV execution and print artifacts
  --write-artifacts  overwrite checked-in first-temporal-timestamp-tz TiKV artifacts after a successful run
...
command=(cargo run -p tiforth-harness-differential --bin first_temporal_timestamp_tz_slice_tikv_live --)
...
tiforth_live_refresh_require_mysql_env "first-temporal-timestamp-tz TiKV live artifact refresh"
"${command[@]}"
