...
Modes:
  default            run live first-filter differential execution and print artifacts
  --write-artifacts  overwrite checked-in inventory artifacts after a successful run
...
prefixes=(TIFORTH_TIDB_MYSQL TIFORTH_TIFLASH_MYSQL)
missing=()
...
if [[ ${#missing[@]} -gt 0 ]]; then
  echo "Missing required env vars for first-filter live artifact refresh:" >&2
  printf '  - %s\n' "${missing[@]}" >&2
  echo "Set them and rerun this script." >&2
  exit 1
fi

command=(cargo run -p tiforth-harness-differential --bin first_filter_is_not_null_live --)
