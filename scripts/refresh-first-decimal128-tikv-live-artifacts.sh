...
usage() {
  cat <<'USAGE'
Usage:
  scripts/refresh-first-decimal128-tikv-live-artifacts.sh [--write-artifacts] [--dry-run]

Modes:
  default            run live first-decimal128 TiKV execution and print artifacts
  --write-artifacts  overwrite checked-in first-decimal128 TiKV artifacts after a successful run
...
command=(cargo run -p tiforth-harness-differential --bin first_decimal128_slice_tikv_live --)
...
tiforth_live_refresh_require_mysql_env "first-decimal128 TiKV live artifact refresh"
"${command[@]}"
