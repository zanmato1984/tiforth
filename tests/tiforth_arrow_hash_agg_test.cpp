...
arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> RunAggPipeline(
  ...
  ARROW_RETURN_NOT_OK(builder->AppendTransform(
      [engine_ptr = engine.get(), keys, aggs]() -> arrow::Result<TransformOpPtr> {
        return std::make_unique<ArrowHashAggTransformOp>(engine_ptr, keys, aggs);
      }));
...
arrow::Status RunArrowHashAggSmoke() {
...
arrow::Status RunArrowHashAggComputedExpr() {
...
arrow::Status RunArrowHashAggStringKey() {
...
arrow::Status RunArrowHashAggMultiKey() {
...
TEST(TiForthArrowHashAggTest, GroupByAndAggregates) { ASSERT_OK(RunArrowHashAggSmoke()); }
TEST(TiForthArrowHashAggTest, ComputedKeyAndArg) { ASSERT_OK(RunArrowHashAggComputedExpr()); }
TEST(TiForthArrowHashAggTest, StringKey) { ASSERT_OK(RunArrowHashAggStringKey()); }
TEST(TiForthArrowHashAggTest, MultiKey) { ASSERT_OK(RunArrowHashAggMultiKey()); }
