...
arrow::Status RunPilotWaitForNotify() {
  ...
  for (int i = 0; i < 8; ++i) {
    ARROW_ASSIGN_OR_RAISE(auto status, group.GetTask()(task_ctx, /*task_id=*/0));
    if (status.IsBlocked()) {
      ...
      auto blocked = std::dynamic_pointer_cast<task::BlockedResumer>(awaiter->resumers()[0]);
      if (blocked == nullptr) {
      ...
      ARROW_ASSIGN_OR_RAISE(auto blocked_again, group.GetTask()(task_ctx, /*task_id=*/0));
