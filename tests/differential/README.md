...
- `crates/tiforth-harness-differential` executes first-expression and first-filter-is-not-null slices through TiDB, TiFlash, and TiKV checkpoints (including TiKV pairwise drift outputs), executes first-temporal-date32, first-decimal128, and first-float64-ordering slices through TiDB and TiFlash adapter cores, validates checked-in paired artifacts under `inventory/`, and executes first-exchange-slice parity checks through `src/first_exchange_slice.rs`
