...
- `crates/tiforth-harness-differential` executes the first-expression, first-filter-is-not-null, first-temporal-date32, and first-decimal128 slices through the current TiDB and TiFlash adapter cores, validates checked-in paired artifacts under `inventory/`, and now executes first-exchange-slice parity checks through `src/first_exchange_slice.rs`
