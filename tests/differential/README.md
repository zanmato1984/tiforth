...
- `tests/differential/first-signed-widening-add-int64-slice.md` defines and anchors executable first differential signed-widening `add<int64>` semantics (exact `int64 + int64` plus the admitted widening pairs `int32 + int64` and `int64 + int32`)
- `tests/differential/first-widening-add-float64-slice.md` defines the first differential docs checkpoint for narrow widening `add<float64>` semantics (exact `float64 + float64` plus the admitted mixed `int32`/`int64` with `float64` pairs)
...
- `tests/differential/first-widening-add-float64-slice-artifacts.md` defines the stable `case-results` and `drift-report` carriers for the first differential widening `add<float64>` slice
...
- `adapters/first-widening-add-float64-slice.md` defines the minimal request and response surface for the first differential widening `add<float64>` slice
...
- `crates/tiforth-harness-differential` executes first-expression, first-unsigned-arithmetic, first-signed-widening-add-int64, first-filter-is-not-null, first-temporal-date32, first-temporal-timestamp-tz, first-decimal128, first-json, first-collation-string, first-struct, first-map, and first-union slices through TiDB/TiFlash checkpoints, carries the documented TiKV single-engine and pairwise checkpoints for the supported slices that already have them, validates checked-in paired and single-engine artifacts under `inventory/`, and executes first-exchange-slice parity checks through `src/first_exchange_slice.rs`
...
- `crates/tiforth-harness-differential/src/bin/first_signed_widening_add_int64_slice.rs` wires a fixture-based artifact refresh path for first-signed-widening-add-int64-slice TiDB-versus-TiFlash artifacts
