...
- the first intentionally broad multi-input operator program should stay docs-first and is currently anchored in `docs/design/first-hash-join-build-probe-boundary.md`
- that program should define operator semantics, build/probe runtime attachment, ownership, admission, collation, adapter, and harness boundaries before a shared-kernel implementation issue starts
- later per-operator semantic specs should live under `docs/spec/operators/` rather than letting one implementation PR become the source of truth
