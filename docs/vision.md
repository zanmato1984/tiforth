...
- the first checked-in per-engine compatibility notes now cover first-expression, first-filter, first-temporal-date32, and first-decimal128 for TiDB, TiFlash, and TiKV, plus first-float64-ordering for TiDB and TiFlash under `inventory/`
...
- the first docs-first TiKV float64 ordering adapter boundary now exists through `adapters/first-float64-ordering-slice-tikv.md` for follow-on single-engine and pairwise TiKV coverage
