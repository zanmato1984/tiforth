...
- the next checkpoint is to execute TiKV first-union-slice single-engine and pairwise differential coverage so the existing union `case_id`, `input_ref`, and artifact carriers extend to TiKV-compatible evidence
- that follow-on checkpoint should keep the current first-union slice identifiers and TiDB/TiFlash artifact carriers stable while adding TiKV compatibility notes and pairwise drift-report artifacts
