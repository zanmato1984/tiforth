...
- the first checked-in per-engine compatibility notes now cover first-expression, first-filter, and first-temporal-date32 slices under `inventory/` for TiDB and TiFlash
...
- the first decimal `decimal128` checkpoint now has docs and local executable conformance coverage through `docs/design/first-decimal-semantic-slice.md`, `tests/conformance/first-decimal128-slice.md`, and `crates/tiforth-kernel/tests/decimal128_slice.rs`, with differential and adapter anchors in `tests/differential/first-decimal128-slice.md` and `adapters/first-decimal128-slice.md`
- the first float64 NaN/infinity ordering checkpoint now has docs anchors and local executable conformance coverage through `docs/design/first-float64-ordering-slice.md`, `tests/conformance/first-float64-ordering-slice.md`, and `crates/tiforth-kernel/tests/float64_slice.rs`, with differential and adapter anchors in `tests/differential/first-float64-ordering-slice.md` and `adapters/first-float64-ordering-slice.md`
