...
- the next checkpoint is to turn one remaining docs-first family into executable local coverage, starting with `first-json-slice` so the documented `case_id`, `input_ref`, and artifact carriers gain a concrete shared-kernel checkpoint
- that follow-on checkpoint should keep the current `case_id`, `input_ref`, and artifact-carrier identifiers stable before broadening JSON path, containment, or ordering scope
