...
- the first checked-in per-engine compatibility notes now cover first-expression, first-filter, first-temporal-date32, first-decimal128, and first-float64-ordering for TiDB, TiFlash, and TiKV under `inventory/`
...
- the first timezone-aware timestamp checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-temporal-timestamp-tz-slice.md`, `tests/conformance/first-temporal-timestamp-tz-slice.md`, `tests/differential/first-temporal-timestamp-tz-slice.md`, and `adapters/first-temporal-timestamp-tz-slice.md`, plus local executable shared-kernel conformance coverage in `crates/tiforth-kernel/tests/temporal_timestamp_tz_slice.rs`
- the first docs-first TiKV timestamp-timezone adapter boundary now exists through `adapters/first-temporal-timestamp-tz-slice-tikv.md` for follow-on single-engine and pairwise TiKV coverage
...
- the first executable TiKV single-engine float64 ordering checkpoint now exists through `crates/tiforth-adapter-tikv`, `crates/tiforth-harness-differential/src/first_float64_ordering_slice_tikv.rs`, and checked-in artifacts `inventory/first-float64-ordering-slice-tikv-case-results.json` plus `inventory/first-float64-ordering-slice-tikv-compat-notes.md`, while TiKV pairwise float64 drift artifacts remain follow-on scope
- the first JSON comparability/cast checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-json-semantic-slice.md`, `tests/conformance/first-json-slice.md`, `tests/differential/first-json-slice.md`, and `adapters/first-json-slice.md`
- the first collation-sensitive string checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-collation-string-slice.md`, `tests/conformance/first-collation-string-slice.md`, `tests/differential/first-collation-string-slice.md`, and `adapters/first-collation-string-slice.md`
- the first nested struct handoff checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-struct-aware-handoff-slice.md`, `tests/conformance/first-struct-slice.md`, `tests/differential/first-struct-slice.md`, and `adapters/first-struct-slice.md`
- the first nested map handoff checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-map-aware-handoff-slice.md`, `tests/conformance/first-map-slice.md`, `tests/differential/first-map-slice.md`, and `adapters/first-map-slice.md`
- the first nested union handoff checkpoint now has docs-first semantic, conformance, differential, and adapter anchors through `docs/design/first-union-aware-handoff-slice.md`, `tests/conformance/first-union-slice.md`, `tests/differential/first-union-slice.md`, and `adapters/first-union-slice.md`
