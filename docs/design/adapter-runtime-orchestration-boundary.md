...
Status: issue #92 design checkpoint
...
For milestone 1, shared docs stop at stable case identifiers, normalized request and response fields, and shared meanings for `tiforth` runtime, admission, and ownership events.
...
Adapters may translate shared case refs into engine-native execution however they need, but they must return the documented normalized `case result` carrier and must not redefine shared semantic IDs or error-class meanings.
...
- whether adapter-visible integrations should reuse `LocalExecutionSnapshot`, translate it, or adopt another callback or event carrier
