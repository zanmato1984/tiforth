...
- filter predicate evaluation for `is_not_null(column(index))` with `int32` predicate input in the first filter slice and `date32` predicate input in the first temporal slice
