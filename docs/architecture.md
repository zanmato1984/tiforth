...
Since then, later issue-scoped checkpoints have widened the current repository shape through the first post-gate filter boundary, the first temporal `date32` and `timestamp_tz(us)` slices, the first `decimal128` and float64-ordering slices, the first `uint64` unsigned arithmetic slice, the first signed-widening `add<int64>` slice, the first JSON passthrough-and-predicate checkpoint, and the first nested struct, map, and union passthrough checkpoints while keeping the shared kernel intentionally narrow and docs-first.
