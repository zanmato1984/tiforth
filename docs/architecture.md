...
Since then, later issue-scoped checkpoints have widened the current repository shape through the first post-gate filter boundary, the first temporal `date32` and `timestamp_tz(us)` slices, the first `decimal128` and float64-ordering slices, the first `uint64` unsigned arithmetic slice, the first signed-widening `add<int64>` and first widening `add<float64>` slices, the first JSON passthrough-and-predicate checkpoint, and the first nested struct, map, and union passthrough checkpoints while keeping the shared kernel intentionally narrow and docs-first.
...
- the first signed-widening `int64` executable conformance extension for passthrough `column(index)` over `int64` plus `add<int64>(lhs, rhs)` over exact `int64 + int64` and the admitted widening pairs `int32 + int64` and `int64 + int32`
- the first widening `float64` executable conformance extension for passthrough `column(index)` over `float64` plus `add<float64>(lhs, rhs)` over exact `float64 + float64` and the admitted widening pairs `int32 + float64`, `int64 + float64`, `float64 + int32`, and `float64 + int64`
...
- expression evaluation only for `column(index)`, `literal<int32>(value)`, and `add<int32>(lhs, rhs)` in the milestone-1 slice, plus narrow `uint64` follow-on coverage for `literal<uint64>(value)` and `add<uint64>(lhs, rhs)`, narrow signed-widening `int64` follow-on coverage for `add<int64>(lhs, rhs)` over exact `int64 + int64` and the admitted `int32`/`int64` widening pairs, and narrow widening `float64` follow-on coverage for `add<float64>(lhs, rhs)` over exact `float64 + float64` and the admitted `int32`/`int64` with `float64` pairs
