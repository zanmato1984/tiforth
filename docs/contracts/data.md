...
Detailed first map-aware handoff slice rationale lives in `docs/design/first-map-aware-handoff-slice.md`.
Detailed first union-aware handoff slice rationale lives in `docs/design/first-union-aware-handoff-slice.md`.
...
- the first map-aware shared handoff checkpoint beyond milestone 1 is defined in `docs/design/first-map-aware-handoff-slice.md`: passthrough of `map<int32, int32?>` through `column(index)` with separate parent-domain, entry-domain, key-domain, and value-domain claim ownership units when lifetimes can diverge
- the first union-aware shared handoff checkpoint beyond milestone 1 is defined in `docs/design/first-union-aware-handoff-slice.md`: passthrough of `dense_union<i:int32, n:int32?>` through `column(index)` with separate type-id-domain, dense-offset-domain, and per-child-domain claim ownership units when lifetimes can diverge
...
## First Map Follow-On Checkpoint

Issue #230 now fixes one post-milestone nested checkpoint in
`docs/design/first-map-aware-handoff-slice.md`.

For that first map slice, shared data-contract scope is intentionally
narrow:

- shared nested handoff admits `map<int32, int32?>` arrays only
- this checkpoint is limited to passthrough `column(index)` over that map shape
- claim tracking keeps parent map storage plus entry, key, and value payloads
  in separate ownership units
- shared differential comparison for this slice uses canonical map-entry array
  carriers with explicit key/value nullability rules
- nested predicates, nested compute families, and broader nested logical types
  remain out of scope until follow-on issues

## First Union Follow-On Checkpoint

Issue #241 now fixes one post-milestone nested checkpoint in
`docs/design/first-union-aware-handoff-slice.md`.

For that first union slice, shared data-contract scope is intentionally
narrow:

- shared nested handoff admits `dense_union<i:int32, n:int32?>` arrays only
- this checkpoint is limited to passthrough `column(index)` over that union shape
- claim tracking keeps type-id-domain storage, dense-offset-domain storage, and
  per-variant child payloads in separate ownership units
- shared differential comparison for this slice uses canonical union carriers
  with stable `tag` and `value` keys plus explicit per-variant nullability
- nested predicates, nested compute families, sparse-union expansion, and
  broader nested logical types remain out of scope until follow-on issues
...
Issue #280 now fixes one timezone-aware timestamp checkpoint in
`docs/design/first-temporal-timestamp-tz-slice.md`.

For that first timestamp-timezone slice, shared data-contract scope is
intentionally narrow:

- shared temporal handoff for this slice admits logical `timestamp_tz(us)`
  arrays
- shared differential comparison for this slice normalizes non-null values to
  UTC epoch-microsecond integers
- equivalent instants represented with different source offsets compare equal
  after normalization
- this slice still does not require one shared timezone-session negotiation API
  or runtime timezone-state surface
- `timestamp` without timezone and non-`us` timestamp units remain out of scope
  until a follow-on issue
...
## First Adapter-Visible Claim-Carrier Follow-On

Issue #274 now fixes one post-milestone ownership-observability checkpoint in
`docs/design/adapter-visible-batch-envelope-claim-carrier.md`.

For that first claim-carrier checkpoint, full batch-envelope `batch_id`,
`origin`, and `claims[]` detail remains optional sidecar evidence until a slice
explicitly requires it, and first-slice adapter `case result` fields remain
unchanged.
...
## First Off-Heap State Ownership Follow-On Checkpoint

Issue #282 now fixes one post-milestone ownership checkpoint in
`docs/design/first-off-heap-state-ownership-boundary.md`.

For that first off-heap checkpoint, shared data-contract scope is intentionally
narrow:

- off-heap resident state means governed admitted bytes that are not currently reachable from live emitted Arrow batch payloads
- off-heap resident state remains governed through live consumer ownership and still follows reserve-before-allocate admission before resident growth
- resident off-heap state that is not batch-reachable must not be attached to outgoing batch `claims[]`; it remains local governed state until release, spill, or later batch materialization
