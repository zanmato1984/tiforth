...
PipelineDrain SortPipeOp::Drain() {
  return [this](const TaskContext&, ThreadId) -> OpResult {
    ...
    return OpOutput::Finished(std::move(out));
  };
}

std::unique_ptr<SourceOp> SortPipeOp::ImplicitSource() { return nullptr; }
