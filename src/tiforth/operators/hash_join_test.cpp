...
arrow::Status RunHashJoinSmoke() {
  ...
  pipe_ops.push_back(std::make_unique<op::HashJoinPipeOp>(engine.get(), std::move(build_batches), key));
...
arrow::Status RunHashJoinTwoKeySmoke() {
  ...
  op::JoinKey key{.left = {"s", "k2"}, .right = {"s", "k2"}};
