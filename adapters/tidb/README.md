...
- `crates/tiforth-adapter-tidb` now encodes first-expression-slice, first-filter-is-not-null-slice, first-temporal-date32-slice, first-temporal-timestamp-tz-slice, first-decimal128-slice, first-float64-ordering-slice, and first-unsigned-arithmetic-slice request catalogs with TiDB-oriented SQL lowering and row / error normalization behind runner boundaries
- `crates/tiforth-harness-differential` now exercises that TiDB adapter core alongside the TiFlash adapter core and validates checked-in pairwise first-expression, first-filter, first-temporal-date32, first-temporal-timestamp-tz, first-decimal, first-float64-ordering, and first-unsigned-arithmetic artifacts under `inventory/`
