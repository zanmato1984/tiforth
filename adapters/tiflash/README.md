...
- `crates/tiforth-adapter-tiflash` now encodes first-expression-slice, first-filter-is-not-null-slice, first-temporal-date32-slice, first-temporal-timestamp-tz-slice, first-decimal128-slice, first-float64-ordering-slice, first-unsigned-arithmetic-slice, first-json-slice, first-collation-string-slice, first-struct-slice, and first-map-slice request catalogs with TiFlash-oriented SQL lowering and row / error normalization behind runner boundaries
- `crates/tiforth-harness-differential` now exercises that TiFlash adapter core alongside the TiDB adapter core and validates checked-in pairwise first-expression, first-filter, first-temporal-date32, first-temporal-timestamp-tz, first-decimal, first-float64-ordering, first-unsigned-arithmetic, first-json, first-collation, first-struct, and first-map artifacts under `inventory/`
