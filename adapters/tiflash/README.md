...
- `crates/tiforth-adapter-tiflash` now encodes first-expression-slice, first-filter-is-not-null-slice, first-temporal-date32-slice, and first-decimal128-slice request catalogs with TiFlash-oriented SQL lowering and row / error normalization behind runner boundaries
- `crates/tiforth-harness-differential` now exercises that TiFlash adapter core alongside the TiDB adapter core and validates checked-in pairwise first-expression, first-filter, first-temporal, and first-decimal artifacts under `inventory/`
- `inventory/first-expression-slice-tiflash-compat-notes.md`, `inventory/first-filter-is-not-null-slice-tiflash-compat-notes.md`, and `inventory/first-temporal-date32-slice-tiflash-compat-notes.md` now capture TiFlash-side compatibility notes for the first-expression, first-filter, and first-temporal executable differential slices
...
- extend TiFlash compatibility notes beyond the current first-expression, first-filter, and first-temporal-date32 slice boundaries as new semantic families land
