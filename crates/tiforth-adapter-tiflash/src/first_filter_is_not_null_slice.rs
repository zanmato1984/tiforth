...
pub struct TiflashFirstFilterIsNotNullSliceAdapter;

impl TiflashFirstFilterIsNotNullSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TiflashFirstFilterIsNotNullSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tiflash_sql_for_each_documented_case() {
        ...
        let plans: Vec<(String, String)> = requests
            .iter()
            .map(|request| {
                let plan = TiflashFirstFilterIsNotNullSliceAdapter::lower_request(request).unwrap();
    ...
    fn execute_rows_normalizes_schema_and_row_count() {
        let request = TiflashFirstFilterIsNotNullSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TiflashFirstFilterIsNotNullSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn lowering_rejects_requests_with_mismatched_spec_refs() {
        let mut request = TiflashFirstFilterIsNotNullSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "all-rows-kept")
            .unwrap();
        request.spec_refs.pop();

        let error = TiflashFirstFilterIsNotNullSliceAdapter::lower_request(&request).unwrap_err();
