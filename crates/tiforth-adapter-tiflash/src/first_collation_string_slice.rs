...
pub struct TiflashFirstCollationStringSliceAdapter;

impl TiflashFirstCollationStringSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TiflashFirstCollationStringSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tiflash_sql_for_documented_case_shapes() {
        ...
        for request in &requests {
            let plan = TiflashFirstCollationStringSliceAdapter::lower_request(request).unwrap();
        ...
        let unknown_collation_plan =
            TiflashFirstCollationStringSliceAdapter::lower_request(unknown_collation_request)
                .unwrap();
        ...
        let ordering_request = requests
            ...
            .unwrap();
        let ordering_plan =
            TiflashFirstCollationStringSliceAdapter::lower_request(ordering_request).unwrap();
        assert!(ordering_plan.sql.contains("ORDER BY LOWER(input_rows.s)"));
    }

    #[test]
    fn execute_rows_normalizes_utf8_and_boolean_schema() {
        let request = TiflashFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TiflashFirstCollationStringSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_collation_error_classes() {
        let missing_request = TiflashFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let missing_result =
            TiflashFirstCollationStringSliceAdapter::execute(&missing_request, &missing_runner)
                .unwrap();
        assert_eq!(
        ...
        let unknown_request = TiflashFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "unknown-collation-error")
            .unwrap();
        ...
        let unknown_result =
            TiflashFirstCollationStringSliceAdapter::execute(&unknown_request, &unknown_runner)
                .unwrap();
        assert_eq!(
        ...
        let unsupported_type_request =
            TiflashFirstCollationStringSliceAdapter::canonical_requests()
                .into_iter()
        ...
        let unsupported_type_result = TiflashFirstCollationStringSliceAdapter::execute(
    ...
    fn lowering_rejects_requests_with_mismatched_collation_ref() {
        let mut request = TiflashFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "utf8-binary-eq-literal-alpha")
            .unwrap();
        request.collation_ref = "unicode_ci".to_string();

        let error = TiflashFirstCollationStringSliceAdapter::lower_request(&request).unwrap_err();
