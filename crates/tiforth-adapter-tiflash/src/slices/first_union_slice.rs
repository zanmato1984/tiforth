...
pub struct TiflashFirstUnionSliceAdapter;

impl TiflashFirstUnionSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TiflashFirstUnionSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tiflash_sql_for_documented_case_shapes() {
        ...
        for request in &requests {
            let plan = TiflashFirstUnionSliceAdapter::lower_request(request).unwrap();
        ...
        let missing_column_plan =
            TiflashFirstUnionSliceAdapter::lower_request(missing_column_request).unwrap();
        assert!(missing_column_plan.sql.contains("__missing_column_1"));
    }

    #[test]
    fn execute_rows_normalizes_union_schema_and_values() {
        let request = TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TiflashFirstUnionSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_union_error_classes() {
        let missing_request = TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-missing-column-error")
            .unwrap();
        ...
        let missing_result =
            TiflashFirstUnionSliceAdapter::execute(&missing_request, &missing_runner).unwrap();
        assert_eq!(
        ...
        let unsupported_request = TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let unsupported_result =
            TiflashFirstUnionSliceAdapter::execute(&unsupported_request, &unsupported_runner)
                .unwrap();
        assert_eq!(
    ...
    fn lowering_rejects_requests_with_mismatched_projection_ref() {
        let mut request = TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-column-passthrough")
            .unwrap();
        request.projection_ref = Some("column-1".to_string());

        let error = TiflashFirstUnionSliceAdapter::lower_request(&request).unwrap_err();
