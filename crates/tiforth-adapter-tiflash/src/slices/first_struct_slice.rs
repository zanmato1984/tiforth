...
pub struct TiflashFirstStructSliceAdapter;

impl TiflashFirstStructSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TiflashFirstStructSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tiflash_sql_for_documented_case_shapes() {
        ...
        for request in &requests {
            let plan = TiflashFirstStructSliceAdapter::lower_request(request).unwrap();
        ...
        let missing_column_plan =
            TiflashFirstStructSliceAdapter::lower_request(missing_column_request).unwrap();
        assert!(missing_column_plan.sql.contains("__missing_column_1"));
    }

    #[test]
    fn execute_rows_normalizes_struct_schema_and_values() {
        let request = TiflashFirstStructSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TiflashFirstStructSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_struct_error_classes() {
        let missing_request = TiflashFirstStructSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "struct-missing-column-error")
            .unwrap();
        ...
        let missing_result =
            TiflashFirstStructSliceAdapter::execute(&missing_request, &missing_runner).unwrap();
        assert_eq!(
        ...
        let unsupported_request = TiflashFirstStructSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let unsupported_result =
            TiflashFirstStructSliceAdapter::execute(&unsupported_request, &unsupported_runner)
                .unwrap();
        assert_eq!(
    ...
    fn lowering_rejects_requests_with_mismatched_projection_ref() {
        let mut request = TiflashFirstStructSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "struct-column-passthrough")
            .unwrap();
        request.projection_ref = Some("column-1".to_string());

        let error = TiflashFirstStructSliceAdapter::lower_request(&request).unwrap_err();
