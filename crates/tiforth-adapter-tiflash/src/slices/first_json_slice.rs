...
pub struct TiflashFirstJsonSliceAdapter;

impl TiflashFirstJsonSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TiflashFirstJsonSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tiflash_sql_for_each_documented_case() {
        ...
        for request in requests {
            let plan = TiflashFirstJsonSliceAdapter::lower_request(&request).unwrap();
            assert!(!plan.sql.is_empty());
        }

        let comparison_request = TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "unsupported-json-ordering-comparison-error")
            .unwrap();
        let comparison_plan =
            TiflashFirstJsonSliceAdapter::lower_request(&comparison_request).unwrap();
        assert!(comparison_plan.sql.contains("input_rows.j < input_rows.j"));

        let cast_request = TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "unsupported-cast-to-json-error")
            .unwrap();
        let cast_plan = TiflashFirstJsonSliceAdapter::lower_request(&cast_request).unwrap();
        assert!(cast_plan.sql.contains("CAST(input_rows.s AS JSON)"));
    }

    #[test]
    fn execute_rows_normalizes_json_schema_and_tokens() {
        let request = TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TiflashFirstJsonSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_missing_column_and_unsupported_json_errors() {
        let missing_request = TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let missing_result =
            TiflashFirstJsonSliceAdapter::execute(&missing_request, &missing_runner).unwrap();
        assert_eq!(
        ...
        let comparison_result =
            TiflashFirstJsonSliceAdapter::execute(&comparison_request, &comparison_runner).unwrap();
        assert_eq!(
        ...
        let cast_result =
            TiflashFirstJsonSliceAdapter::execute(&cast_request, &cast_runner).unwrap();
        assert_eq!(
            cast_result.outcome,
            CaseOutcome::Error {
    ...
    fn lowering_rejects_requests_with_mismatched_operation_refs() {
        let mut request = TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "unsupported-json-cast-error")
            .unwrap();
        ...
        let error = TiflashFirstJsonSliceAdapter::lower_request(&request).unwrap_err();
