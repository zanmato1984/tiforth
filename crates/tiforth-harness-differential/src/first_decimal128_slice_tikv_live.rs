...
pub fn render_live_drift_report_markdown(report: &first_decimal128_slice::DriftReport) -> String {
    let rendered = first_decimal128_slice_tikv_pairwise::render_drift_report_markdown(report)
        .replacen(
            "Status: issue #284 follow-on harness checkpoint",
            "Status: issue #284 follow-on harness checkpoint, issue #378 live-runner checkpoint",
            1,
        );
...
fn expected_row_case_schema(case_id: &str) -> Option<Vec<ExpectedColumn>> {
    match case_id {
        "decimal128-column-passthrough" | "decimal128-is-not-null-all-kept" => {
            Some(vec![ExpectedColumn {
                engine_type: "decimal(10,2)",
                nullable: false,
            }])
        }
        "decimal128-column-null-preserve"
        | "decimal128-is-not-null-all-dropped"
        | "decimal128-is-not-null-mixed-keep-drop" => Some(vec![ExpectedColumn {
            engine_type: "decimal(10,2)",
            nullable: true,
        }]),
        _ => None,
...
mod tests {
    ...
    use super::*;
    use tiforth_adapter_tidb::first_decimal128_slice::TidbRunner;
    use tiforth_adapter_tiflash::first_decimal128_slice::TiflashRunner;
    use tiforth_adapter_tikv::first_decimal128_slice::TikvRunner;

    static ENV_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

    #[test]
    fn parse_mysql_batch_output_preserves_decimal_strings() {
        ...
        assert_eq!(
            parsed.rows,
            vec![
                vec![Value::String("1.00".to_string())],
                vec![Value::Null],
                vec![Value::String("-3.75".to_string())],
            ]
        );
    }
    ...
    fn build_engine_columns_requires_known_row_case() {
        let error = build_engine_columns("decimal128-missing-column-error", &[String::from("d")])
            .unwrap_err();

        assert_eq!(
            error,
            "case `decimal128-missing-column-error` returned rows but does not define a row schema in this slice"
        );
    }
    ...
    fn live_drift_renderer_relabels_the_evidence_source_block() {
        let report = first_decimal128_slice::DriftReport {
        ...
        assert!(rendered.contains("issue #378 live-runner checkpoint"));
    ...
    fn tidb_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tidb::TidbFirstDecimal128SliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "decimal128-column-passthrough")
            .unwrap();
        let plan = tidb::TidbFirstDecimal128SliceAdapter::lower_request(&request).unwrap();
    ...
    fn tiflash_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tiflash::TiflashFirstDecimal128SliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "decimal128-column-passthrough")
            .unwrap();
        let plan = tiflash::TiflashFirstDecimal128SliceAdapter::lower_request(&request).unwrap();
    ...
    fn tikv_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tikv::TikvFirstDecimal128SliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "decimal128-column-passthrough")
            .unwrap();
        let plan = tikv::TikvFirstDecimal128SliceAdapter::lower_request(&request).unwrap();
