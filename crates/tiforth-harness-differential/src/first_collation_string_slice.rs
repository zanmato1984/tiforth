...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstCollationStringSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_collation_string_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result =
            tidb::TidbFirstCollationStringSliceAdapter::execute(&tidb_request, tidb_runner)
                .map_err(|error| HarnessError::TidbAdapterValidation {
                    case_id: request.case_id.clone(),
                    error: format!("{error:?}"),
                })?;
        let tiflash_result = tiflash::TiflashFirstCollationStringSliceAdapter::execute(
            &tiflash_request,
            tiflash_runner,
        )
    ...
    let tidb_case_results = CaseResultsArtifact {
        slice_id: tidb::FIRST_COLLATION_STRING_SLICE_ID.to_string(),
        engine: tidb::TIDB_ENGINE.to_string(),
        adapter: tidb::TIDB_ADAPTER.to_string(),
        cases: tidb_cases,
    };
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_COLLATION_STRING_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Collation String Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #358 harness checkpoint\n\n");
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-collation-string-slice-artifacts.md`\n\n");
...
fn outcome_kind_name(outcome: &CaseOutcome) -> &'static str {
    match outcome {
        ...
        CaseOutcome::Error { error_class, .. } => match error_class {
            ...
            ErrorClass::UnknownCollation => "`error/unknown_collation`",
            ErrorClass::UnsupportedCollationType => "`error/unsupported_collation_type`",
...
fn error_class_name(error_class: ErrorClass) -> &'static str {
    match error_class {
        ...
        ErrorClass::UnknownCollation => "unknown_collation",
        ErrorClass::UnsupportedCollationType => "unsupported_collation_type",
...
impl From<tidb::AdapterRequest> for CanonicalRequest {
    fn from(value: tidb::AdapterRequest) -> Self {
        Self {
            ...
            collation_ref: value.collation_ref,
...
fn convert_tidb_error_class(error_class: tidb::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tidb::ErrorClass::UnknownCollation => ErrorClass::UnknownCollation,
        tidb::ErrorClass::UnsupportedCollationType => ErrorClass::UnsupportedCollationType,
...
fn convert_tiflash_error_class(error_class: tiflash::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tiflash::ErrorClass::UnknownCollation => ErrorClass::UnknownCollation,
        tiflash::ErrorClass::UnsupportedCollationType => ErrorClass::UnsupportedCollationType,
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle =
            execute_first_collation_string_slice(&FixtureTidbRunner, &FixtureTiflashRunner)
                .unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "utf8-unicode-ci-ordering-normalization".to_string(),
            spec_refs: vec!["tests/differential/first-collation-string-slice.md".to_string()],
            input_ref: "first-utf8-ordering".to_string(),
            collation_ref: "unicode_ci".to_string(),
            projection_ref: None,
            filter_ref: None,
            comparison_ref: None,
            ordering_ref: Some("order-by-column-0-asc".to_string()),
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unknown-collation-error".to_string(),
            spec_refs: vec!["tests/differential/first-collation-string-slice.md".to_string()],
            input_ref: "first-utf8-basic".to_string(),
            collation_ref: "unknown-collation".to_string(),
            projection_ref: None,
            filter_ref: None,
            comparison_ref: Some("eq-column-0-literal-alpha".to_string()),
            ordering_ref: None,
        ...
        let tiflash_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("unknown collation".to_string()),
            },
        };
