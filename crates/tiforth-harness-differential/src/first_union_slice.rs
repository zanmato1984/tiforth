...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-union-slice-tidb-case-results.json";
pub const TIFLASH_CASE_RESULTS_REF: &str = "inventory/first-union-slice-tiflash-case-results.json";
pub const DRIFT_REPORT_REF: &str = "inventory/first-union-slice-tidb-vs-tiflash-drift-report.md";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_union_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result = tidb::TidbFirstUnionSliceAdapter::execute(&tidb_request, tidb_runner)
            .map_err(|error| HarnessError::TidbAdapterValidation {
                case_id: request.case_id.clone(),
                error: format!("{error:?}"),
            })?;
        let tiflash_result =
            tiflash::TiflashFirstUnionSliceAdapter::execute(&tiflash_request, tiflash_runner)
    ...
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_UNION_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Union Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #366 harness checkpoint\n\n");
...
fn canonicalize_union_cell(value: &Value) -> Value {
    match value {
        ...
        Value::Object(object) => {
            ...
            for (key, value) in entries {
                canonical.insert(key.clone(), canonicalize_union_cell(value));
            }

            Value::Object(canonical)
        }
        Value::Array(values) => Value::Array(values.iter().map(canonicalize_union_cell).collect()),
        other => other.clone(),
    }
}
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_union_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
