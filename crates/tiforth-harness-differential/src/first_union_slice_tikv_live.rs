...
use crate::pairwise::first_union_slice_tikv_pairwise;
use crate::slices::first_union_slice;
...
pub fn render_live_drift_report_markdown(report: &first_union_slice::DriftReport) -> String {
    let rendered = first_union_slice_tikv_pairwise::render_drift_report_markdown(report).replacen(
        "Status: issue #368 harness checkpoint",
        "Status: issue #368 harness checkpoint, issue #374 live-runner checkpoint",
        1,
    );
...
fn parse_mysql_cell(cell: &str) -> Value {
    if cell == "NULL" || cell == "\\N" {
        Value::Null
    } else if starts_json_value(cell) {
...
fn starts_json_value(cell: &str) -> bool {
    matches!(cell.as_bytes().first(), Some(b'{') | Some(b'['))
}
...
fn expected_row_case_schema(case_id: &str) -> Option<Vec<ExpectedColumn>> {
    match case_id {
        ...
        | "union-variant-null-preserve" => Some(vec![ExpectedColumn {
            logical_type: "dense_union<i:int32,n:int32?>",
            nullable: false,
        }]),
        _ => None,
    }
...
mod tests {
    ...
    use super::*;
    use tiforth_adapter_tidb::first_union_slice::TidbRunner;
    use tiforth_adapter_tiflash::first_union_slice::TiflashRunner;
    use tiforth_adapter_tikv::first_union_slice::TikvRunner;

    static ENV_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

    #[test]
    fn parse_mysql_batch_output_supports_union_json_rows() {
        let parsed = parse_mysql_batch_output(
            "u\n{\"tag\":\"i\",\"value\":1}\n{\"tag\":\"n\",\"value\":null}\n",
        )
        .unwrap();

        assert_eq!(parsed.headers, vec!["u"]);
        assert_eq!(
            parsed.rows,
            vec![
                vec![serde_json::json!({"tag": "i", "value": 1})],
                vec![serde_json::json!({"tag": "n", "value": Value::Null})],
            ]
        );
    }
    ...
    fn build_engine_columns_requires_known_row_case() {
        let error =
            build_engine_columns("union-missing-column-error", &[String::from("u")]).unwrap_err();

        assert_eq!(
            error,
            "case `union-missing-column-error` returned rows but does not define a row schema in this slice"
        );
    }
    ...
    fn live_drift_renderer_relabels_the_evidence_source_block() {
        let report = first_union_slice::DriftReport {
            slice_id: "first-union-slice".to_string(),
        ...
        assert!(rendered.contains("issue #374 live-runner checkpoint"));
    ...
    fn tidb_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tidb::TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-column-passthrough")
            .unwrap();
        let plan = tidb::TidbFirstUnionSliceAdapter::lower_request(&request).unwrap();
    ...
    fn tiflash_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tiflash::TiflashFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-column-passthrough")
            .unwrap();
        let plan = tiflash::TiflashFirstUnionSliceAdapter::lower_request(&request).unwrap();
    ...
    fn tikv_live_runner_reports_adapter_unavailable_when_config_is_missing() {
        ...
        let request = tikv::TikvFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-column-passthrough")
            .unwrap();
        let plan = tikv::TikvFirstUnionSliceAdapter::lower_request(&request).unwrap();
