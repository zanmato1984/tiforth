...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-decimal128-slice-tidb-case-results.json";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstDecimal128SliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstDecimal128SliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_decimal128_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result =
            tidb::TidbFirstDecimal128SliceAdapter::execute(&tidb_request, tidb_runner).map_err(
                |error| HarnessError::TidbAdapterValidation {
                    case_id: request.case_id.clone(),
                    error: format!("{error:?}"),
                },
            )?;
        let tiflash_result =
            tiflash::TiflashFirstDecimal128SliceAdapter::execute(&tiflash_request, tiflash_runner)
    ...
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_DECIMAL128_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Decimal128 Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #206 harness checkpoint\n\n");
...
fn matching_rows_summary(case_id: &str, schema: &[SchemaField], row_count: usize) -> String {
    match schema {
        ...
        _ => format!("TiDB and TiFlash both returned matching row output for `{case_id}`."),
    }
}
...
fn unsupported_follow_up(engine: &str, case_id: &str) -> Option<String> {
    ...
    } else if case_id == "invalid-decimal-metadata-error" {
        Some(format!(
            "Decide whether {engine} should normalize `{case_id}` as `invalid_decimal_metadata` or remain explicitly unsupported."
        ))
    } else {
        Some(format!(
...
fn convert_tidb_error_class(error_class: tidb::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tidb::ErrorClass::UnsupportedDecimalType => ErrorClass::UnsupportedDecimalType,
        tidb::ErrorClass::InvalidDecimalMetadata => ErrorClass::InvalidDecimalMetadata,
...
fn convert_tiflash_error_class(error_class: tiflash::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tiflash::ErrorClass::UnsupportedDecimalType => ErrorClass::UnsupportedDecimalType,
        tiflash::ErrorClass::InvalidDecimalMetadata => ErrorClass::InvalidDecimalMetadata,
...
mod tests {
    ...
    fn checked_in_artifacts_match_the_fixture_harness_output() {
        let bundle =
            execute_first_decimal128_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!("1.00")], vec![json!("2.50")]],
                row_count: 2,
            },
        };
        let tiflash_case = CaseResult {
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!("1.00")], vec![json!("9.99")]],
                row_count: 2,
            },
            ..tidb_case
                .clone()
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unsupported-decimal-type-error".to_string(),
            spec_refs: vec!["tests/differential/first-decimal128-slice.md".to_string()],
        ...
        let tiflash_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("unsupported decimal type".to_string()),
            },
        };
            ...
            match plan.request.case_id.as_str() {
                ...
                "unsupported-decimal-type-error" => {
                    Err(tidb::EngineExecutionError::EngineFailure {
                        ...
                        message: "unsupported decimal type: decimal256 input is out of scope for first-decimal128-slice".to_string(),
                    })
                }
                "invalid-decimal-metadata-error" => {
                    Err(tidb::EngineExecutionError::EngineFailure {
                        code: Some("1105".to_string()),
                        message: "invalid decimal metadata: decimal(10,12) has scale greater than precision".to_string(),
                    })
                }
