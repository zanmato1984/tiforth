...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-map-slice-tidb-case-results.json";
pub const TIFLASH_CASE_RESULTS_REF: &str = "inventory/first-map-slice-tiflash-case-results.json";
pub const DRIFT_REPORT_REF: &str = "inventory/first-map-slice-tidb-vs-tiflash-drift-report.md";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> = tidb::TidbFirstMapSliceAdapter::canonical_requests()
        .into_iter()
        .map(CanonicalRequest::from)
        .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstMapSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_map_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result = tidb::TidbFirstMapSliceAdapter::execute(&tidb_request, tidb_runner)
            .map_err(|error| HarnessError::TidbAdapterValidation {
                case_id: request.case_id.clone(),
                error: format!("{error:?}"),
            })?;
        let tiflash_result =
            tiflash::TiflashFirstMapSliceAdapter::execute(&tiflash_request, tiflash_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Map Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #362 harness checkpoint\n\n");
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-map-slice-artifacts.md`\n\n");
...
fn canonicalize_map_cell(value: &Value) -> Value {
    match value {
        ...
        Value::Object(object) => {
            ...
            for (key, value) in entries {
                canonical.insert(key.clone(), canonicalize_map_cell(value));
            }

            Value::Object(canonical)
        }
        Value::Array(values) => Value::Array(values.iter().map(canonicalize_map_cell).collect()),
        other => other.clone(),
    }
}
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_map_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            spec_refs: vec!["tests/differential/first-map-slice.md".to_string()],
