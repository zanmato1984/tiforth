...
pub fn canonical_requests() -> Vec<tikv::AdapterRequest> {
    tikv::TikvFirstFilterIsNotNullSliceAdapter::canonical_requests()
}

pub fn execute_first_filter_is_not_null_slice_tikv<R: tikv::TikvRunner>(
    tikv_runner: &R,
    ...
    for request in requests {
        ...
        let result = tikv::TikvFirstFilterIsNotNullSliceAdapter::execute(&request, tikv_runner)
...
mod tests {
    ...
    fn checked_in_artifact_matches_fixture_harness_output() {
        let artifact = execute_first_filter_is_not_null_slice_tikv(&FixtureTikvRunner).unwrap();
    ...
    impl tikv::TikvRunner for FixtureTikvRunner {
        fn run(
            ...
            match plan.request.case_id.as_str() {
                ...
                "all-rows-dropped" => Ok(rows_result(
                    vec![tikv::EngineColumn {
                        name: "a".to_string(),
                        engine_type: "int".to_string(),
                        nullable: true,
                ...
                "mixed-keep-drop" => Ok(rows_result(
                    vec![
                        tikv::EngineColumn {
                            name: "a".to_string(),
                            engine_type: "int".to_string(),
