...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-expression-slice-tidb-case-results.json";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstExpressionSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstExpressionSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_expression_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result =
            tidb::TidbFirstExpressionSliceAdapter::execute(&tidb_request, tidb_runner).map_err(
        ...
        let tiflash_result =
            tiflash::TiflashFirstExpressionSliceAdapter::execute(&tiflash_request, tiflash_runner)
    ...
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_EXPRESSION_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Expression Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #113 harness checkpoint\n\n");
...
fn compare_case_results(tidb_case: &CaseResult, tiflash_case: &CaseResult) -> DriftCase {
    ...
    match (&tidb_case.outcome, &tiflash_case.outcome) {
        ...
        (
            ...
            let drift_dimensions =
                row_drift_dimensions(tidb_schema, tiflash_schema, tidb_rows, tiflash_rows, *tidb_row_count, *tiflash_row_count);
            ...
            DriftCase {
                ...
                evidence_refs,
                follow_up: matching_rows_follow_up(&tidb_case.case_id, tidb_schema),
            }
        }
        (
            CaseOutcome::Error {
...
fn matching_rows_follow_up(case_id: &str, schema: &[SchemaField]) -> Option<String> {
    match schema {
        [field]
            if matches!(
                case_id,
                "literal-int32-seven" | "literal-int32-null"
...
mod tests {
    ...
    fn checked_in_artifacts_match_the_fixture_harness_output() {
        let bundle =
            execute_first_expression_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            projection_ref: "column-a".to_string(),
