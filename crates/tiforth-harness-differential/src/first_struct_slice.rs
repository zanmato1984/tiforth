...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-struct-slice-tidb-case-results.json";
pub const TIFLASH_CASE_RESULTS_REF: &str = "inventory/first-struct-slice-tiflash-case-results.json";
pub const DRIFT_REPORT_REF: &str = "inventory/first-struct-slice-tidb-vs-tiflash-drift-report.md";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstStructSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstStructSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_struct_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result = tidb::TidbFirstStructSliceAdapter::execute(&tidb_request, tidb_runner)
            .map_err(|error| HarnessError::TidbAdapterValidation {
                case_id: request.case_id.clone(),
                error: format!("{error:?}"),
            })?;
        let tiflash_result =
            tiflash::TiflashFirstStructSliceAdapter::execute(&tiflash_request, tiflash_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Struct Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #360 harness checkpoint\n\n");
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-struct-slice-artifacts.md`\n\n");
            ...
            row.iter()
                ...
                .map(|(index, value)| match schema.get(index) {
                    Some(field) if field.logical_type.starts_with("struct<") => {
                        canonicalize_struct_cell(value)
                    }
                    _ => value.clone(),
                })
                .collect()
...
fn canonicalize_struct_cell(value: &Value) -> Value {
    match value {
        ...
        Value::Object(object) => {
            ...
            for (key, value) in entries {
                canonical.insert(key.clone(), canonicalize_struct_cell(value));
            }

            Value::Object(canonical)
        }
        Value::Array(values) => Value::Array(values.iter().map(canonicalize_struct_cell).collect()),
        other => other.clone(),
    }
}
...
fn outcome_kind_name(outcome: &CaseOutcome) -> &'static str {
    match outcome {
        ...
        CaseOutcome::Error { error_class, .. } => match error_class {
            ...
            ErrorClass::UnsupportedNestedFamily => "`error/unsupported_nested_family`",
...
fn convert_tidb_error_class(error_class: tidb::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tidb::ErrorClass::UnsupportedNestedFamily => ErrorClass::UnsupportedNestedFamily,
...
fn convert_tiflash_error_class(error_class: tiflash::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tiflash::ErrorClass::UnsupportedNestedFamily => ErrorClass::UnsupportedNestedFamily,
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_struct_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            spec_refs: vec!["tests/differential/first-struct-slice.md".to_string()],
            input_ref: "first-struct-basic".to_string(),
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unsupported-nested-family-error".to_string(),
            spec_refs: vec!["tests/differential/first-struct-slice.md".to_string()],
            input_ref: "first-map-basic".to_string(),
            projection_ref: Some("column-0".to_string()),
        ...
        let tiflash_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("unsupported nested family".to_string()),
            },
        };
