...
use tiforth_adapter_tidb::first_decimal128_slice as tidb;
...
use crate::slices::first_decimal128_slice::{
    self, execute_first_decimal128_slice,
...
use crate::tikv::first_decimal128_slice_tikv::{
    self, execute_first_decimal128_slice_tikv, CaseResultsArtifact as TikvCaseResultsArtifact,
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let shared_requests: Vec<CanonicalRequest> = first_decimal128_slice::canonical_requests()
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?
        .into_iter()
        .map(CanonicalRequest::from)
        .collect();
    let tikv_requests: Vec<CanonicalRequest> = first_decimal128_slice_tikv::canonical_requests()
        .into_iter()
        .map(CanonicalRequest::from)
        .collect();
...
pub fn execute_first_decimal128_slice_tikv_pairwise<T, F, K>(
    tidb_runner: &T,
    tiflash_runner: &F,
    tikv_runner: &K,
    ...
    K: tikv::TikvRunner,
{
    ...
    let tidb_tiflash_bundle = execute_first_decimal128_slice(tidb_runner, tiflash_runner)
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?;
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-decimal128-slice-artifacts.md`\n\n");
...
fn build_pairwise_drift_report(
    ...
    Ok(DriftReport {
        slice_id: tidb::FIRST_DECIMAL128_SLICE_ID.to_string(),
        engines: vec![
...
fn unsupported_follow_up(engine: &str, case_id: &str) -> Option<String> {
    if case_id == "unsupported-decimal-type-error" {
        Some(format!(
            "Decide whether {engine} should normalize `{case_id}` as `unsupported_decimal_type` or remain explicitly unsupported."
        ))
    } else {
        Some(format!(
...
fn outcome_kind_name(outcome: &CaseOutcome) -> &'static str {
    match outcome {
        ...
        CaseOutcome::Error { error_class, .. } => match error_class {
            ...
            ErrorClass::UnsupportedDecimalType => "`error/unsupported_decimal_type`",
            ErrorClass::InvalidDecimalMetadata => "`error/invalid_decimal_metadata`",
...
fn error_class_name(error_class: ErrorClass) -> &'static str {
    match error_class {
        ...
        ErrorClass::UnsupportedDecimalType => "unsupported_decimal_type",
        ErrorClass::InvalidDecimalMetadata => "invalid_decimal_metadata",
...
fn convert_tikv_error_class(error_class: tikv::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tikv::ErrorClass::UnsupportedDecimalType => ErrorClass::UnsupportedDecimalType,
        tikv::ErrorClass::InvalidDecimalMetadata => ErrorClass::InvalidDecimalMetadata,
        tikv::ErrorClass::AdapterUnavailable => ErrorClass::AdapterUnavailable,
        tikv::ErrorClass::EngineError => ErrorClass::EngineError,
    }
}

impl From<first_decimal128_slice::CanonicalRequest> for CanonicalRequest {
    fn from(value: first_decimal128_slice::CanonicalRequest) -> Self {
        Self {
            slice_id: value.slice_id,
            case_id: value.case_id,
            spec_refs: value.spec_refs,
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_decimal128_slice_tikv_pairwise(
            &FixtureTidbRunner,
    ...
    #[test]
    fn compare_case_results_marks_logical_type_difference_as_drift() {
        ...
        let right_case = row_case_result("tikv", "decimal128-column-passthrough", "int32", false);
        ...
        assert_eq!(
            case.comparison_dimensions,
            vec![ComparisonDimension::LogicalType]
        );
        assert!(case.summary.contains("disagree on `logical_type`"));
    }
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        ...
        assert_eq!(
            case.follow_up,
            Some(
                "Decide whether tidb should normalize `unsupported-decimal-type-error` as `unsupported_decimal_type` or remain explicitly unsupported.".to_string()
            )
        );
    }
    ...
    fn row_case_result(
        ...
        CaseResult {
            slice_id: "first-decimal128-slice".to_string(),
            engine: engine.to_string(),
            adapter: format!("{engine}-sql"),
            case_id: case_id.to_string(),
            spec_refs: vec!["tests/differential/first-decimal128-slice.md".to_string()],
            input_ref: "first-decimal128-basic".to_string(),
    ...
    fn error_case_result(engine: &str, case_id: &str, error_class: ErrorClass) -> CaseResult {
        CaseResult {
            ...
            case_id: case_id.to_string(),
            spec_refs: vec!["tests/differential/first-decimal128-slice.md".to_string()],
            input_ref: "first-decimal256-basic".to_string(),
            projection_ref: Some("column-0".to_string()),
            filter_ref: None,
    ...
    impl tidb::TidbRunner for FixtureTidbRunner {
        fn run(
            ...
            match plan.request.case_id.as_str() {
                "decimal128-column-passthrough" => Ok(rows_result_tidb(
                    vec![tidb::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: false,
                    }],
                    vec![vec![json!("1.00")], vec![json!("2.50")], vec![json!("-3.75")]],
                )),
                "decimal128-column-null-preserve" => Ok(rows_result_tidb(
                    vec![tidb::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-all-kept" => Ok(rows_result_tidb(
                    vec![tidb::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: false,
                ...
                "decimal128-is-not-null-all-dropped" => Ok(rows_result_tidb(
                    vec![tidb::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-mixed-keep-drop" => Ok(rows_result_tidb(
                    vec![tidb::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
    ...
    impl tiflash::TiflashRunner for FixtureTiflashRunner {
        fn run(
            ...
            match plan.request.case_id.as_str() {
                "decimal128-column-passthrough" => Ok(rows_result_tiflash(
                ...
                "decimal128-column-null-preserve" => Ok(rows_result_tiflash(
                    vec![tiflash::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-all-kept" => Ok(rows_result_tiflash(
                    vec![tiflash::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: false,
                ...
                "decimal128-is-not-null-all-dropped" => Ok(rows_result_tiflash(
                    vec![tiflash::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-mixed-keep-drop" => Ok(rows_result_tiflash(
                    vec![tiflash::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
    ...
    impl tikv::TikvRunner for FixtureTikvRunner {
        fn run(
            ...
            match plan.request.case_id.as_str() {
                "decimal128-column-passthrough" => Ok(rows_result_tikv(
                ...
                "decimal128-column-null-preserve" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-all-kept" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: false,
                ...
                "decimal128-is-not-null-all-dropped" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-is-not-null-mixed-keep-drop" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "d".to_string(),
                        engine_type: "decimal(10,2)".to_string(),
                        nullable: true,
                ...
                "decimal128-missing-column-error" => Err(tikv::EngineExecutionError::EngineFailure {
                    code: Some("1054".to_string()),
