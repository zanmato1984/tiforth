...
use crate::slices::first_expression_slice::{
    self, execute_first_expression_slice,
...
use crate::tikv::first_expression_slice_tikv::{
    self, execute_first_expression_slice_tikv, CaseResultsArtifact as TikvCaseResultsArtifact,
...
pub enum HarnessError {
    ...
    CaseIdentityMismatch {
        ...
        left_projection_ref: String,
        right_projection_ref: String,
    },
}
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let shared_requests: Vec<CanonicalRequest> = first_expression_slice::canonical_requests()
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?
        .into_iter()
        .map(CanonicalRequest::from)
        .collect();
    let tikv_requests: Vec<CanonicalRequest> = first_expression_slice_tikv::canonical_requests()
        .into_iter()
        .map(CanonicalRequest::from)
        .collect();
...
pub fn execute_first_expression_slice_tikv_pairwise<T, F, K>(
    tidb_runner: &T,
    tiflash_runner: &F,
    tikv_runner: &K,
    ...
    K: tikv::TikvRunner,
{
    ...
    let tidb_tiflash_bundle = execute_first_expression_slice(tidb_runner, tiflash_runner)
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?;
    let tikv_case_results = execute_first_expression_slice_tikv(tikv_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-expression-slice-artifacts.md`\n\n");
...
fn build_pairwise_drift_report(
    ...
    Ok(DriftReport {
        slice_id: tidb::FIRST_EXPRESSION_SLICE_ID.to_string(),
        engines: vec![
    ...
    match (&left_case.outcome, &right_case.outcome) {
        ...
        (
            ...
            DriftCase {
                ...
                evidence_refs,
                follow_up: matching_rows_follow_up(&left_case.case_id, left_schema),
            }
        }
        (
            CaseOutcome::Error {
...
fn matching_rows_follow_up(case_id: &str, schema: &[SchemaField]) -> Option<String> {
    match schema {
        [field]
            if matches!(case_id, "literal-int32-seven" | "literal-int32-null")
                && field.logical_type != "int32" =>
        {
            Some(format!(
                "Both adapters currently surface `{case_id}` as `{}`; later issues can decide whether to narrow that metadata back to the shared `int32` contract.",
                field.logical_type
            ))
        }
        _ => None,
...
fn unsupported_follow_up(engine: &str, case_id: &str) -> Option<String> {
    if case_id == "add-int32-overflow-error" {
        Some(format!(
            "Decide whether {engine} should add a shared-`int32` overflow strategy for `{case_id}` or remain explicitly unsupported."
        ))
    } else {
        Some(format!(
...
fn convert_tikv_error_class(error_class: tikv::ErrorClass) -> ErrorClass {
    match error_class {
        tikv::ErrorClass::ArithmeticOverflow => ErrorClass::ArithmeticOverflow,
        tikv::ErrorClass::AdapterUnavailable => ErrorClass::AdapterUnavailable,
        tikv::ErrorClass::EngineError => ErrorClass::EngineError,
    }
}

impl From<first_expression_slice::CanonicalRequest> for CanonicalRequest {
    fn from(value: first_expression_slice::CanonicalRequest) -> Self {
        Self {
            slice_id: value.slice_id,
            case_id: value.case_id,
            spec_refs: value.spec_refs,
...
mod tests {
    ...
    fn compare_case_results_marks_logical_type_difference_as_drift() {
        let left_case = CaseResult {
            ...
            case_id: "add-int32-literal".to_string(),
            spec_refs: vec!["tests/differential/first-expression-slice.md".to_string()],
            input_ref: "first-expression-slice-int32-basic".to_string(),
            projection_ref: "add-a-plus-one".to_string(),
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let left_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "add-int32-overflow-error".to_string(),
            spec_refs: vec!["tests/differential/first-expression-slice.md".to_string()],
            input_ref: "first-expression-slice-int32-overflow".to_string(),
            projection_ref: "add-a-plus-one".to_string(),
        ...
        let right_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("value is out of range".to_string()),
            },
        };
