...
pub const TIDB_CASE_RESULTS_REF: &str = "inventory/first-json-slice-tidb-case-results.json";
pub const TIFLASH_CASE_RESULTS_REF: &str = "inventory/first-json-slice-tiflash-case-results.json";
pub const DRIFT_REPORT_REF: &str = "inventory/first-json-slice-tidb-vs-tiflash-drift-report.md";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstJsonSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_json_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    for request in &requests {
        ...
        let tidb_result = tidb::TidbFirstJsonSliceAdapter::execute(&tidb_request, tidb_runner)
            .map_err(|error| HarnessError::TidbAdapterValidation {
                case_id: request.case_id.clone(),
                error: format!("{error:?}"),
            })?;
        let tiflash_result =
            tiflash::TiflashFirstJsonSliceAdapter::execute(&tiflash_request, tiflash_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First JSON Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #356 harness checkpoint\n\n");
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-json-slice-artifacts.md`\n\n");
...
fn row_drift_dimensions(
    ...
    let tidb_canonical_rows = canonical_rows_for_comparison(tidb_rows, tidb_schema);
    let tiflash_canonical_rows = canonical_rows_for_comparison(tiflash_rows, tiflash_schema);
    if tidb_canonical_rows != tiflash_canonical_rows {
...
fn canonical_rows_for_comparison(rows: &[Vec<Value>], schema: &[SchemaField]) -> Vec<Vec<Value>> {
    rows.iter()
        .map(|row| {
            row.iter()
                .enumerate()
                .map(|(index, value)| match schema.get(index) {
                    Some(field) if field.logical_type == "json" => canonicalize_json_cell(value),
                    _ => value.clone(),
                })
                .collect()
        })
...
fn canonicalize_json_cell(value: &Value) -> Value {
    match value {
...
fn outcome_kind_name(outcome: &CaseOutcome) -> &'static str {
    match outcome {
        ...
        CaseOutcome::Error { error_class, .. } => match error_class {
            ...
            ErrorClass::UnsupportedJsonComparison => "`error/unsupported_json_comparison`",
            ErrorClass::UnsupportedJsonCast => "`error/unsupported_json_cast`",
...
fn error_class_name(error_class: ErrorClass) -> &'static str {
    match error_class {
        ...
        ErrorClass::UnsupportedJsonComparison => "unsupported_json_comparison",
        ErrorClass::UnsupportedJsonCast => "unsupported_json_cast",
...
impl From<&CanonicalRequest> for tidb::AdapterRequest {
    fn from(value: &CanonicalRequest) -> Self {
        Self {
            ...
            comparison_ref: value.comparison_ref.clone(),
            cast_ref: value.cast_ref.clone(),
        }
    }
}
...
fn convert_tidb_error_class(error_class: tidb::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tidb::ErrorClass::UnsupportedJsonComparison => ErrorClass::UnsupportedJsonComparison,
        tidb::ErrorClass::UnsupportedJsonCast => ErrorClass::UnsupportedJsonCast,
...
fn convert_tiflash_error_class(error_class: tiflash::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tiflash::ErrorClass::UnsupportedJsonComparison => ErrorClass::UnsupportedJsonComparison,
        tiflash::ErrorClass::UnsupportedJsonCast => ErrorClass::UnsupportedJsonCast,
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_json_slice(&FixtureTidbRunner, &FixtureTiflashRunner).unwrap();
    ...
    #[test]
    fn compare_case_results_normalizes_json_object_key_order_as_match() {
        let tidb_case = CaseResult {
            slice_id: tidb::FIRST_JSON_SLICE_ID.to_string(),
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "json-object-canonicalization".to_string(),
            spec_refs: vec!["tests/differential/first-json-slice.md".to_string()],
            input_ref: "first-json-nullable".to_string(),
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "json-array-order-preserved".to_string(),
            spec_refs: vec!["tests/differential/first-json-slice.md".to_string()],
            input_ref: "first-json-array-order".to_string(),
            projection_ref: Some("column-0".to_string()),
            filter_ref: None,
            comparison_ref: None,
            cast_ref: None,
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!("[1,2]")], vec![json!("[2,1]")]],
                row_count: 2,
            },
        };
        let tiflash_case = CaseResult {
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!("[1,2]")], vec![json!("[1,2]")]],
                row_count: 2,
            },
            ..tidb_case
                .clone()
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unsupported-cast-to-json-error".to_string(),
            spec_refs: vec!["tests/differential/first-json-slice.md".to_string()],
            input_ref: "first-utf8-json-text".to_string(),
            projection_ref: None,
            filter_ref: None,
            comparison_ref: None,
            cast_ref: Some("utf8-column-0-to-json".to_string()),
        ...
        let tiflash_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("unsupported json cast".to_string()),
            },
        };
    ...
    pub(crate) struct FixtureTidbRunner;
    ...
    pub(crate) struct FixtureTiflashRunner;
