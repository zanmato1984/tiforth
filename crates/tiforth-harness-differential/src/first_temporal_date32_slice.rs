...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstTemporalDate32SliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstTemporalDate32SliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_temporal_date32_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_TEMPORAL_DATE32_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Temporal Date32 Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #187 harness checkpoint\n\n");
...
mod tests {
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        ...
        let tiflash_case = CaseResult {
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!(0)], vec![json!(99)], vec![json!(2)]],
                row_count: 3,
            },
            ..tidb_case
                .clone()
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unsupported-temporal-type-error".to_string(),
            spec_refs: vec!["tests/differential/first-temporal-date32-slice.md".to_string()],
