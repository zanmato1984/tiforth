...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let tidb_requests: Vec<CanonicalRequest> =
        tidb::TidbFirstTemporalTimestampTzSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
    let tiflash_requests: Vec<CanonicalRequest> =
        tiflash::TiflashFirstTemporalTimestampTzSliceAdapter::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_temporal_timestamp_tz_slice<T, F>(
    tidb_runner: &T,
    tiflash_runner: &F,
    ...
    F: tiflash::TiflashRunner,
{
    ...
    let tiflash_case_results = CaseResultsArtifact {
        slice_id: tiflash::FIRST_TEMPORAL_TIMESTAMP_TZ_SLICE_ID.to_string(),
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Temporal Timestamp-Timezone Slice TiDB-vs-TiFlash Drift Report\n\n");
    rendered.push_str("Status: issue #304 harness checkpoint\n\n");
...
fn convert_tidb_error_class(error_class: tidb::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tidb::ErrorClass::UnsupportedTemporalType => ErrorClass::UnsupportedTemporalType,
        tidb::ErrorClass::UnsupportedTemporalUnit => ErrorClass::UnsupportedTemporalUnit,
...
fn convert_tiflash_error_class(error_class: tiflash::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tiflash::ErrorClass::UnsupportedTemporalType => ErrorClass::UnsupportedTemporalType,
        tiflash::ErrorClass::UnsupportedTemporalUnit => ErrorClass::UnsupportedTemporalUnit,
...
mod tests {
    ...
    fn checked_in_artifacts_match_the_fixture_harness_output() {
        let bundle =
            execute_first_temporal_timestamp_tz_slice(&FixtureTidbRunner, &FixtureTiflashRunner)
                .unwrap();
    ...
    fn compare_case_results_marks_row_differences_as_drift() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "timestamp-tz-column-passthrough".to_string(),
            spec_refs: vec!["tests/differential/first-temporal-timestamp-tz-slice.md".to_string()],
            input_ref: "first-temporal-timestamp-tz-basic".to_string(),
        ...
        let tiflash_case = CaseResult {
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!(0)], vec![json!(99)], vec![json!(2_000_000)]],
                row_count: 3,
            },
            ..tidb_case
                .clone()
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        let tidb_case = CaseResult {
            ...
            engine: tidb::TIDB_ENGINE.to_string(),
            adapter: tidb::TIDB_ADAPTER.to_string(),
            case_id: "unsupported-temporal-timestamp-without-timezone-error".to_string(),
            spec_refs: vec!["tests/differential/first-temporal-timestamp-tz-slice.md".to_string()],
            input_ref: "first-temporal-timestamp-naive-basic".to_string(),
            projection_ref: Some("column-0".to_string()),
            filter_ref: None,
            ordering_ref: None,
        ...
        let tiflash_case = CaseResult {
            ...
            outcome: CaseOutcome::Error {
                ...
                engine_message: Some("unsupported temporal type".to_string()),
            },
        };
