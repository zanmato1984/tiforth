...
use crate::slices::first_temporal_date32_slice::{
    self, execute_first_temporal_date32_slice,
...
use crate::tikv::first_temporal_date32_slice_tikv::{
    self, execute_first_temporal_date32_slice_tikv, CaseResultsArtifact as TikvCaseResultsArtifact,
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let shared_requests: Vec<CanonicalRequest> = first_temporal_date32_slice::canonical_requests()
    ...
    let tikv_requests: Vec<CanonicalRequest> =
        first_temporal_date32_slice_tikv::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_temporal_date32_slice_tikv_pairwise<T, F, K>(
    tidb_runner: &T,
    tiflash_runner: &F,
    tikv_runner: &K,
    ...
    K: tikv::TikvRunner,
{
    ...
    let tidb_tiflash_bundle = execute_first_temporal_date32_slice(tidb_runner, tiflash_runner)
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?;
    let tikv_case_results = execute_first_temporal_date32_slice_tikv(tikv_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("- the stable artifact-carrier boundary lives in `tests/differential/first-temporal-date32-slice-artifacts.md`\n\n");
...
fn build_pairwise_drift_report(
    ...
    Ok(DriftReport {
        slice_id: tidb::FIRST_TEMPORAL_DATE32_SLICE_ID.to_string(),
        engines: vec![
...
fn unsupported_follow_up(engine: &str, case_id: &str) -> Option<String> {
    if case_id == "unsupported-temporal-type-error" {
        Some(format!(
...
impl From<first_temporal_date32_slice::CanonicalRequest> for CanonicalRequest {
    fn from(value: first_temporal_date32_slice::CanonicalRequest) -> Self {
        Self {
            slice_id: value.slice_id,
            case_id: value.case_id,
            spec_refs: value.spec_refs,
...
mod tests {
    ...
    fn compare_case_results_marks_logical_type_difference_as_drift() {
        let left_case = row_case_result("tidb", "date32-column-passthrough", "date32", false);
        let right_case = row_case_result("tikv", "date32-column-passthrough", "int32", false);
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        ...
        assert_eq!(
            case.follow_up,
            Some(
                "Decide whether tidb should normalize `unsupported-temporal-type-error` as `unsupported_temporal_type` or remain explicitly unsupported.".to_string()
            )
        );
    }
    ...
    fn row_case_result(
        ...
        CaseResult {
            ...
            input_ref: "first-temporal-date32-basic".to_string(),
            ...
            outcome: CaseOutcome::Rows {
                ...
                rows: vec![vec![json!(0)], vec![json!(1)], vec![json!(2)]],
                row_count: 3,
            },
        }
    }
    ...
    fn error_case_result(engine: &str, case_id: &str, error_class: ErrorClass) -> CaseResult {
        CaseResult {
            ...
            case_id: case_id.to_string(),
            spec_refs: vec!["tests/differential/first-temporal-date32-slice.md".to_string()],
            input_ref: "first-temporal-timestamp-basic".to_string(),
            projection_ref: Some("column-0".to_string()),
            filter_ref: None,
            ...
            match plan.request.case_id.as_str() {
                ...
                "unsupported-temporal-type-error" => {
                    Err(tidb::EngineExecutionError::EngineFailure {
                        ...
                        message: "unsupported temporal type: timestamp input is out of scope for first-temporal-date32-slice"
                            .to_string(),
                    })
                }
