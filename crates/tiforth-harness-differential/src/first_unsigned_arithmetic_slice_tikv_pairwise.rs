...
pub const TIDB_VS_TIKV_DRIFT_REPORT_SIDECAR_REF: &str =
    "inventory/first-unsigned-arithmetic-slice-tidb-vs-tikv-drift-report.json";
...
pub const TIFLASH_VS_TIKV_DRIFT_REPORT_SIDECAR_REF: &str =
    "inventory/first-unsigned-arithmetic-slice-tiflash-vs-tikv-drift-report.json";
...
pub fn canonical_requests() -> Result<Vec<CanonicalRequest>, HarnessError> {
    let shared_requests: Vec<CanonicalRequest> =
        first_unsigned_arithmetic_slice::canonical_requests()
    ...
    let tikv_requests: Vec<CanonicalRequest> =
        first_unsigned_arithmetic_slice_tikv::canonical_requests()
            .into_iter()
            .map(CanonicalRequest::from)
            .collect();
...
pub fn execute_first_unsigned_arithmetic_slice_tikv_pairwise<T, F, K>(
    tidb_runner: &T,
    tiflash_runner: &F,
    tikv_runner: &K,
    ...
    K: tikv::TikvRunner,
{
    ...
    let tidb_tiflash_bundle = execute_first_unsigned_arithmetic_slice(tidb_runner, tiflash_runner)
        .map_err(|error| HarnessError::TidbTiflashAdapterValidation { error })?;
    let tikv_case_results = execute_first_unsigned_arithmetic_slice_tikv(tikv_runner)
...
pub fn render_drift_report_markdown(report: &DriftReport) -> String {
    ...
    rendered.push_str("# First Unsigned Arithmetic Slice ");
    rendered.push_str(&left_display);
    rendered.push_str("-vs-");
    rendered.push_str(&right_display);
    rendered.push_str(" Drift Report\n\n");
    rendered.push_str("Status: issue #328 follow-on harness checkpoint\n\n");
    ...
    match (&left_case.outcome, &right_case.outcome) {
        ...
        (
            ...
            DriftCase {
                ...
                evidence_refs,
                follow_up: if status == DriftStatus::Match {
                    None
                } else {
                    Some(
        ...
        (left_outcome, right_outcome) => DriftCase {
            ...
            comparison_dimensions: vec![ComparisonDimension::ErrorClass],
            summary: outcome_mismatch_summary(
                left_engine,
                right_engine,
                &left_case.case_id,
                left_outcome,
            ...
            evidence_refs,
            follow_up: Some(
                "Decide whether the mismatched outcome should become comparable or stay explicitly unsupported."
                    .to_string(),
            ),
        },
    }
...
fn unsupported_follow_up(engine: &str, case_id: &str) -> Option<String> {
    if case_id == "unsupported-unsigned-family-error" {
        Some(format!(
            "Decide whether {engine} should normalize `{case_id}` as `unsupported_unsigned_family` or remain explicitly unsupported."
        ))
    } else {
        Some(format!(
...
fn convert_tikv_schema_field(value: tikv::SchemaField) -> SchemaField {
    SchemaField {
        name: value.name,
...
fn convert_tikv_error_class(error_class: tikv::ErrorClass) -> ErrorClass {
    match error_class {
        ...
        tikv::ErrorClass::UnsignedOverflow => ErrorClass::UnsignedOverflow,
        tikv::ErrorClass::MixedSignedUnsigned => ErrorClass::MixedSignedUnsigned,
        tikv::ErrorClass::UnsupportedUnsignedFamily => ErrorClass::UnsupportedUnsignedFamily,
        tikv::ErrorClass::AdapterUnavailable => ErrorClass::AdapterUnavailable,
        tikv::ErrorClass::EngineError => ErrorClass::EngineError,
    }
}

impl From<first_unsigned_arithmetic_slice::CanonicalRequest> for CanonicalRequest {
    fn from(value: first_unsigned_arithmetic_slice::CanonicalRequest) -> Self {
        Self {
            slice_id: value.slice_id,
            case_id: value.case_id,
            spec_refs: value.spec_refs,
...
mod tests {
    ...
    fn checked_in_artifacts_match_fixture_harness_output() {
        let bundle = execute_first_unsigned_arithmetic_slice_tikv_pairwise(
            &FixtureTidbRunner,
    ...
    fn compare_case_results_marks_logical_type_difference_as_drift() {
        let left_case = row_case_result("tidb", "uint64-column-passthrough", "uint64", false);
        let right_case = row_case_result("tikv", "uint64-column-passthrough", "int64", false);
        ...
        assert_eq!(
            case.summary,
            "TiDB and TiKV disagree on `logical_type` for `uint64-column-passthrough` under `row-order-preserved`."
        );
    }
    ...
    fn compare_case_results_marks_adapter_unavailable_as_unsupported() {
        ...
        assert_eq!(
            case.follow_up,
            Some(
                "Decide whether tidb should normalize `unsupported-unsigned-family-error` as `unsupported_unsigned_family` or remain explicitly unsupported.".to_string()
            )
        );
    }
    ...
    fn row_case_result(
        ...
        CaseResult {
            slice_id: "first-unsigned-arithmetic-slice".to_string(),
            engine: engine.to_string(),
            adapter: format!("{engine}-sql"),
            case_id: case_id.to_string(),
            spec_refs: vec!["tests/differential/first-unsigned-arithmetic-slice.md".to_string()],
            input_ref: "first-uint64-basic".to_string(),
            comparison_mode: "row-order-preserved".to_string(),
    ...
    impl tikv::TikvRunner for FixtureTikvRunner {
        fn run(
            ...
            match plan.request.case_id.as_str() {
                "uint64-column-passthrough" => Ok(rows_result_tikv(
                ...
                "uint64-literal-projection" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "seven".to_string(),
                        engine_type: "bigint unsigned".to_string(),
                        nullable: false,
                ...
                "uint64-add-basic" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "sum".to_string(),
                        engine_type: "bigint unsigned".to_string(),
                        nullable: false,
                ...
                "uint64-add-null-propagation" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "sum".to_string(),
                        engine_type: "bigint unsigned".to_string(),
                        nullable: true,
                ...
                "uint64-is-not-null-mixed-keep-drop" => Ok(rows_result_tikv(
                    vec![tikv::EngineColumn {
                        name: "u".to_string(),
                        engine_type: "bigint unsigned".to_string(),
                        nullable: true,
