...
pub struct TidbFirstUnionSliceAdapter;

impl TidbFirstUnionSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TidbFirstUnionSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tidb_sql_for_documented_case_shapes() {
        ...
        for request in &requests {
            let plan = TidbFirstUnionSliceAdapter::lower_request(request).unwrap();
        ...
        let missing_column_plan =
            TidbFirstUnionSliceAdapter::lower_request(missing_column_request).unwrap();
        assert!(missing_column_plan.sql.contains("__missing_column_1"));
    }

    #[test]
    fn execute_rows_normalizes_union_schema_and_values() {
        let request = TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TidbFirstUnionSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_union_error_classes() {
        let missing_request = TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-missing-column-error")
            .unwrap();
        ...
        let missing_result =
            TidbFirstUnionSliceAdapter::execute(&missing_request, &missing_runner).unwrap();
        assert_eq!(
        ...
        let unsupported_request = TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let unsupported_result =
            TidbFirstUnionSliceAdapter::execute(&unsupported_request, &unsupported_runner).unwrap();
        assert_eq!(
    ...
    fn lowering_rejects_requests_with_mismatched_projection_ref() {
        let mut request = TidbFirstUnionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "union-column-passthrough")
            .unwrap();
        request.projection_ref = Some("column-1".to_string());

        let error = TidbFirstUnionSliceAdapter::lower_request(&request).unwrap_err();
