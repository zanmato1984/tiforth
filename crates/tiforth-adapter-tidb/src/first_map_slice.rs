...
pub struct TidbFirstMapSliceAdapter;

impl TidbFirstMapSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TidbFirstMapSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tidb_sql_for_documented_case_shapes() {
        ...
        for request in &requests {
            let plan = TidbFirstMapSliceAdapter::lower_request(request).unwrap();
        ...
        let missing_column_plan =
            TidbFirstMapSliceAdapter::lower_request(missing_column_request).unwrap();
        assert!(missing_column_plan.sql.contains("__missing_column_1"));
    }

    #[test]
    fn execute_rows_normalizes_map_schema_and_values() {
        let request = TidbFirstMapSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TidbFirstMapSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn execute_normalizes_map_error_classes() {
        let missing_request = TidbFirstMapSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "map-missing-column-error")
            .unwrap();
        ...
        let missing_result =
            TidbFirstMapSliceAdapter::execute(&missing_request, &missing_runner).unwrap();
        assert_eq!(
        ...
        let unsupported_request = TidbFirstMapSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let unsupported_result =
            TidbFirstMapSliceAdapter::execute(&unsupported_request, &unsupported_runner).unwrap();
        assert_eq!(
    ...
    fn lowering_rejects_requests_with_mismatched_projection_ref() {
        let mut request = TidbFirstMapSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "map-column-passthrough")
            .unwrap();
        request.projection_ref = Some("column-1".to_string());

        let error = TidbFirstMapSliceAdapter::lower_request(&request).unwrap_err();
