...
pub struct TikvFirstFloat64OrderingSliceAdapter;

impl TikvFirstFloat64OrderingSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TikvFirstFloat64OrderingSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tikv_sql_for_each_documented_case() {
        ...
        let plans: Vec<(String, String)> = requests
            .iter()
            .map(|request| {
                let plan = TikvFirstFloat64OrderingSliceAdapter::lower_request(request).unwrap();
    ...
    fn execute_rows_normalizes_schema_and_row_count() {
        let request = TikvFirstFloat64OrderingSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TikvFirstFloat64OrderingSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn lowering_rejects_requests_with_mismatched_operation_refs() {
        let mut request = TikvFirstFloat64OrderingSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let error = TikvFirstFloat64OrderingSliceAdapter::lower_request(&request).unwrap_err();
