...
pub struct TikvFirstFilterIsNotNullSliceAdapter;

impl TikvFirstFilterIsNotNullSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TikvFirstFilterIsNotNullSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tikv_sql_for_each_documented_case() {
        ...
        let plans: Vec<(String, String)> = requests
            .iter()
            .map(|request| {
                let plan = TikvFirstFilterIsNotNullSliceAdapter::lower_request(request).unwrap();
    ...
    fn execute_rows_normalizes_schema_and_row_count() {
        let request = TikvFirstFilterIsNotNullSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TikvFirstFilterIsNotNullSliceAdapter::execute(&request, &runner).unwrap();
    ...
    fn lowering_rejects_requests_with_mismatched_spec_refs() {
        let mut request = TikvFirstFilterIsNotNullSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "all-rows-kept")
            .unwrap();
        request.spec_refs.pop();

        let error = TikvFirstFilterIsNotNullSliceAdapter::lower_request(&request).unwrap_err();
