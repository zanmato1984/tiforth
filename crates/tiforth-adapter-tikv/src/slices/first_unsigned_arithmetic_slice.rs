...
pub struct TikvFirstUnsignedArithmeticSliceAdapter;

impl TikvFirstUnsignedArithmeticSliceAdapter {
...
fn is_missing_column(engine_code: Option<&str>, engine_message: &str) -> bool {
    ...
    engine::is_missing_column(engine_code, engine_message)
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests();
    ...
    fn execute_rows_normalizes_schema_and_row_count() {
        let request = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "uint64-add-null-propagation")
            .unwrap();
        ...
        let result = TikvFirstUnsignedArithmeticSliceAdapter::execute(&request, &runner).unwrap();
        ...
        assert_eq!(
            ...
            CaseOutcome::Rows {
                ...
                rows: vec![vec![Value::Null], vec![Value::Null], vec![json!("7")]],
                row_count: 3,
            }
        );
    }
    ...
    #[test]
    fn execute_normalizes_unsigned_overflow_errors() {
        let request = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "uint64-add-overflow-error")
            .unwrap();
        ...
        assert_eq!(
            result.outcome,
            CaseOutcome::Error {
                error_class: ErrorClass::UnsignedOverflow,
    ...
    #[test]
    fn execute_normalizes_mixed_signed_unsigned_errors() {
        let request = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "mixed-signed-unsigned-arithmetic-error")
            .unwrap();
        let runner = StubRunner::error(EngineExecutionError::EngineFailure {
        ...
        assert_eq!(
            result.outcome,
            CaseOutcome::Error {
                error_class: ErrorClass::MixedSignedUnsigned,
                engine_code: Some("1105".to_string()),
                engine_message: Some(
                    "mixed signed and unsigned arithmetic is unsupported for first-unsigned-arithmetic-slice; got Int64"
                        .to_string()
                ),
            }
        );
    ...
    #[test]
    fn execute_normalizes_unsupported_unsigned_family_errors() {
        let request = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "unsupported-unsigned-family-error")
            .unwrap();
        let runner = StubRunner::error(EngineExecutionError::EngineFailure {
            code: Some("1105".to_string()),
        ...
        assert_eq!(
            ...
            CaseOutcome::Error {
                ...
                engine_code: Some("1105".to_string()),
                engine_message: Some(
                    "unsupported unsigned type: uint32 input is out of scope for first-unsigned-arithmetic-slice"
                        .to_string()
                ),
            }
        );
    ...
    fn lowering_rejects_requests_with_mismatched_operation_refs() {
        let mut request = TikvFirstUnsignedArithmeticSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let error = TikvFirstUnsignedArithmeticSliceAdapter::lower_request(&request).unwrap_err();
