...
pub struct TikvFirstExpressionSliceAdapter;

impl TikvFirstExpressionSliceAdapter {
...
mod tests {
    ...
    fn canonical_requests_cover_all_documented_cases() {
        let requests = TikvFirstExpressionSliceAdapter::canonical_requests();
    ...
    fn lowering_renders_tikv_sql_for_each_documented_case() {
        ...
        let plans: Vec<(String, String)> = requests
            .iter()
            .map(|request| {
                let plan = TikvFirstExpressionSliceAdapter::lower_request(request).unwrap();
    ...
    fn execute_rows_normalizes_schema_and_row_count() {
        let request = TikvFirstExpressionSliceAdapter::canonical_requests()
            .into_iter()
        ...
        let result = TikvFirstExpressionSliceAdapter::execute(&request, &runner).unwrap();
    ...
    #[test]
    fn execute_normalizes_tikv_overflow_errors() {
        let request = TikvFirstExpressionSliceAdapter::canonical_requests()
            .into_iter()
    ...
    fn execute_normalizes_adapter_unavailable_without_extra_code() {
        ...
        assert_eq!(
            ...
            CaseOutcome::Error {
                error_class: ErrorClass::AdapterUnavailable,
                engine_code: None,
                engine_message: Some("TiKV DSN is not configured".to_string()),
            }
        );
    ...
    fn lowering_rejects_requests_with_mismatched_spec_refs() {
        let mut request = TikvFirstExpressionSliceAdapter::canonical_requests()
            .into_iter()
            .find(|request| request.case_id == "column-passthrough")
            .unwrap();
        request.spec_refs.pop();

        let error = TikvFirstExpressionSliceAdapter::lower_request(&request).unwrap_err();
