# First Filter Slice TiFlash Compatibility Notes
...
- `crates/tiforth-adapter-tiflash/src/first_filter_is_not_null_slice.rs`
...
- the TiFlash adapter lowers documented first-filter requests into SQL with a
  `WHERE ... IS NOT NULL` predicate over adapter-owned input aliases
- the first-filter checked-in TiFlash case-results artifact includes explicit
  row outcomes for all kept, all dropped, and mixed keep/drop paths
...
- TiFlash normalized row outcomes preserve documented field order and names for
  all row-returning first-filter cases
- TiFlash normalized outcomes preserve shared logical type names (`int32`) and
...
- current checked-in TiFlash artifact reports `engine_code = 1054` and message
...
- this note records TiFlash-side compatibility evidence only for the
  first-filter `is_not_null(column(index))` slice
