# First Temporal Timestamp-Timezone Slice TiDB Compatibility Notes
...
- `crates/tiforth-adapter-tidb/src/first_temporal_timestamp_tz_slice.rs`
...
- the TiDB adapter lowers timestamp-timezone passthrough requests into SQL
  projection over adapter-owned aliases
- checked-in TiDB case-results include non-null, nullable, and
...
- the TiDB adapter lowers first timestamp-timezone filter requests into SQL
  with `WHERE ... IS NOT NULL`
- checked-in TiDB case-results include all kept, all dropped, and mixed
  keep/drop paths
