# First Float64 Ordering Slice TiDB Compatibility Notes
...
- `crates/tiforth-adapter-tidb/src/first_float64_ordering_slice.rs`
...
- the TiDB adapter lowers float64 passthrough requests into SQL projection over
  adapter-owned aliases
- checked-in TiDB float64 case-results include both ordinary finite values and
...
- the TiDB adapter lowers first-float64 filter requests into SQL with
  `WHERE ... IS NOT NULL`
- checked-in TiDB float64 case-results include all-kept and mixed keep/drop
  paths
...
- the first float64 slice carries `comparison_mode` in each request and case
  result
- the TiDB adapter supports both documented modes:
  `row-order-preserved` and `float64-multiset-canonical`

#### Recorded TiDB Facts

- `float64-canonical-ordering-normalization` records 6 rows in TiDB output as
  `"NaN"`, `"1.0"`, `"-Infinity"`, `"Infinity"`, `"0.0"`, `"-0.0"`
- the paired drift report still classifies this case as `match` under
