# First Decimal128 Slice TiDB Compatibility Notes
...
- `crates/tiforth-adapter-tidb/src/first_decimal128_slice.rs`
...
- the TiDB adapter lowers decimal passthrough requests into SQL projection over
  adapter-owned aliases
- checked-in TiDB decimal case-results include both non-null and nullable
  passthrough paths
...
- the TiDB adapter lowers first-decimal filter requests into SQL with
  `WHERE ... IS NOT NULL`
- checked-in TiDB decimal case-results include all kept, all dropped, and mixed
  keep/drop paths
...
- `invalid-decimal-metadata-error` normalizes to
  `error_class = invalid_decimal_metadata`
- current checked-in TiDB artifact reports `engine_code = 1105` with message
