# First Temporal Date32 Slice TiFlash Compatibility Notes
...
- `crates/tiforth-adapter-tiflash/src/first_temporal_date32_slice.rs`
...
- the TiFlash adapter lowers temporal passthrough requests into SQL projection
  over adapter-owned aliases
- checked-in TiFlash temporal case-results include both non-null and nullable
  passthrough paths
...
- the TiFlash adapter lowers first-temporal filter requests into SQL with
  `WHERE ... IS NOT NULL`
- checked-in TiFlash temporal case-results include all kept, all dropped, and
