# First Unsigned Arithmetic Slice TiDB Compatibility Notes
...
- the TiDB adapter lowers first-unsigned passthrough and literal projection
  requests into SQL projection over adapter-owned aliases
- checked-in TiDB case-results preserve `uint64` values as canonical base-10
...
- the TiDB adapter lowers first-unsigned add probes into row-wise SQL
...
- the TiDB adapter lowers the first-unsigned filter probe into SQL
  `WHERE ... IS NOT NULL`
...
- TiDB normalization maps overflow signals into
...
- `uint64-add-overflow-error` normalizes to
  `error_class = unsigned_overflow`
- current checked-in TiDB artifact reports `engine_code = 1690` with message
...
- TiDB normalization records this checkpoint as shared
...
- TiDB normalization maps this case into shared
